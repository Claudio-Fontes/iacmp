...
10. Aguardar stacks (sem poll loop longo — verificar a cada 2-3min, máx 30min total)
    **Se uma stack falhar:** antes de destruir tudo, avalie:
    - Identificar quais stacks ainda não deployaram por depender da que falhou
    - Re-deployar SÓ a stack que falhou: `iacmp deploy --stack <nome> --provider <cloud>`
    - Em seguida, re-deployar as dependentes em ordem (se o iacmp não continuar automaticamente)
    - Só destrua tudo e recomeça se o estado ficou irrecuperável (RG corrompido, falha exige mudança no synth)
