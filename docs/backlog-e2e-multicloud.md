...
- **Export cross-stack AWS frágil em regeneração (PENDENTE, dono cloudformation-expert).** Regenerar o projeto mudando a topologia de export (ex: 1 role compartilhada → N roles por handler) trava o deploy incremental: o CFN recusa remover/alterar um export em uso (`Cannot delete export … as it is in use`). O iacmp deploya a exportadora ANTES da importadora (ordem de criação) — errada pra REMOVER um export. O Azure (`_main.bicep`, deployment único, refs simbólicas) NÃO sofre: reprocessa tudo junto → o mesmo projeto é mais resiliente a regeneração no Azure (achado de arquitetura). Opções: (a) pré-flight que compara exports ativos vs template novo e, se um export em uso vai sumir, ABORTA com orientação clara ("não é update incremental — rode `iacmp destroy && iacmp deploy`"); (b) deploy em 2 fases (atualiza importadora primeiro). Mínimo viável = (a) detectar+avisar. Workaround atual: destroy+recreate (funcionou na sessão).
