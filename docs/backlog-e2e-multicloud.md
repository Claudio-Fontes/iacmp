...
- ✅ **FEITO (2026-07-24) — Convenção de stacks: SEPARADAS por domínio em AMBAS as clouds.** Coberto em 3 camadas: (a) `battery-tester.md` tem a regra INEGOCIÁVEL ("stacks sempre separadas por domínio, NUNCA main-stack.ts", com tabela de subpastas); (b) template `fullstack` do `iacmp init` separado em 4 stacks (grupo 1, `b3bc6ef`); (c) validador anti-monólito `validateStackDomainSeparation` REJEITA no synth stack com ≥4 domínios (grupo 1). O `_main.bicep` segue sendo OUTPUT do synth Azure, não input.
...
- ✅ **FEITO (2026-07-23, iacmp `f569c4e`) — APIM compartilhado (`azureSharedApim`).** `iacmp.json` → `azure.sharedApim: { name, resourceGroup }`. Presente → synth Azure referencia o APIM `existing` (não cria `Microsoft.ApiManagement/service`) e cria só os filhos (apis/backends/operations/policies/namedValues) prefixados pelo projeto (nome ARM + path no gateway). Ausente → comportamento atual (regressão zero: 144 testes + golden inalterados). Destroy: como o APIM vira `existing` (fora da deployment stack), o `deleteAll` já não o toca — só as APIs do projeto saem. Validado com `az bicep build`. **Cross-RG (APIM em RG dedicado) ✅ FEITO (2026-07-24, `02b6f63`)**: filhos num MÓDULO Bicep aninhado (arquivo-irmão `_*.apim-shared-children.bicep`) com `scope: resourceGroup(<rg-do-apim>)` — resolve o BCP165; `az bicep build` limpo; jest azure 144/144, CLI 56/56; cenários sem/same-RG byte-a-byte iguais. Modo cross-RG é EXCLUSIVO de testes/bateria (decisão do usuário). Limitação: JWT authorizer (Secret.Vault) + cross-RG lança erro explícito em synth-time (wiring de identity entre escopos não feito); destroy cross-RG a validar em e2e real (deployment stack rastreia recursos cross-RG por design). Elimina o piso de ~30-45min de APIM por projeto.
...
- ✅ **FEITO (2026-07-24, iacmp `cbebb2a`) — `destroy` reporta falha prematura em Lambda-em-VPC.** O waiter do CLI (`aws cloudformation wait stack-delete-complete`) desiste em ~15min; o cleanup de ENI de Lambda-em-VPC passa disso e o destroy reportava falha enquanto o CFN ainda deletava, sem disparar a próxima stack (a VPC que a database importa — precisou de `delete-stack` manual). Fix: `retries: 3` no waiter — cada re-execução continua aguardando o `DELETE_IN_PROGRESS` até completar (~1h de tolerância) e só então a próxima stack é destruída; `DELETE_FAILED` real é terminal → falha rápido. Resolve o timeout prematuro E a ordem das dependentes.

- ✅ **RESOLVIDO (2026-07-24) — Exemplos AWS do KB com padrões que o validador rejeitava.** Já saneado: `'Host'` HOJE é atributo válido de `Cache.Redis` (`core/src/refs.ts`: `Endpoint|Port|Host|ConnectionString`); nenhum exemplo do corpus usa `ref(...,'Host')` de verdade (só menção em nota) e não há `TABLE_NAME` literal. O corpus está limpo (harness 126/126) desde a fonte única (`packages/knowledge/src/corpus`). Backlog estava desatualizado.
...
- ✅ **FEITO (2026-07-24, iacmp `5cb11c5`) — SNS→SQS fan-out cross-stack.** O `AWS::SQS::QueuePolicy` montava `Queues` com `Ref` local cru (hardcoded same-stack); agora usa `resolveRef(ref(qId,'QueueUrl'),ctx)` — o MESMO resolvedor cross-stack do `Endpoint`. Same-stack segue `Ref` local (idêntico, sem regressão); cross-stack vira `Fn::ImportValue` do export `-QueueUrl`. Fixture `aws-sns-fanout` voltou a ser 2 stacks separadas (workaround desfeito). +2 testes (refs.test.ts); harness 126/126.
