...
- **Loop de aprendizado: knowledge base auto-enriquecida por deploy real** (ideia do usuário, 2026-07-23) — HOJE não existe: infra inédita que passa synth+deploy+teste NÃO volta pra base; o conhecimento se perde e o próximo pedido parecido começa do zero. Decidido (2026-07-23) em DOIS modos opt-in, "dar pra receber":
  - **MODO 1 — base local isolada (EM IMPLEMENTAÇÃO):** cliente auto-alimenta APENAS o próprio `~/.iacmp/knowledge.db`; cresce só com o próprio uso; recebe os curados oficiais via npm, mas NÃO os crowd-sourced de outros. Tudo client-side, sem servidor. Fluxo: captura pós-deploy bem-sucedido → generaliza → dedup contra o corpus local → gate pelo HARNESS (synth verde) → upsert no banco com `origin='local'` (camada separada dos curados-oficiais). Opt-in via `iacmp.json` (default off). Peças já prontas: banco read-write, harness, ensureSeeded, flag validated.
