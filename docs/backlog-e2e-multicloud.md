...
- 🟡 **PARCIAL (2026-07-22, iacmp-mcp master `2e9a395`) — Testes de contrato dos exemplos do knowledge base** — harness FEITO: `iacmp-mcp/test/contract-battery.mjs` (`npm run test:contract`) roda `iacmp synth` real sobre cada exemplo + 10 invariantes semânticas (tipo de recurso, connection string, env vars, refs, wiring, gates de cloud) + cobertura da matriz; modos `curados` e `db`. Achou 112/242 quebrados e o corpus foi saneado para 126/126. **FALTA só o gate de CI** (rodar em push/PR) — próximo passo recomendado. Mata a classe "dessincronização" que dominou 2026-07-22.
