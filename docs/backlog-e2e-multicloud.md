...
- **Distribuição da knowledge base para o cliente (npm install)** — HOJE quebrado: o corpus (126 exemplos) + o seeder (`migrateStatic`) vivem só no iacmp-mcp; a CLI publicada (`iacmp` 2.2.2) só LÊ `~/.iacmp/knowledge.db` (readonly) e retorna null se o arquivo não existe → `iacmp ai` do cliente roda SEM exemplos. `@iacmp/knowledge` nem está publicado (404), então `@iacmp/mcp` também não instala limpo. Fix: (a) bundlar o corpus na CLI publicada + rodar o seed no primeiro uso (mover/espelhar migrateStatic para a CLI), OU (b) publicar `@iacmp/knowledge` carregando lib+corpus e semear on-first-run. É a metade "distribuição" da fonte única (achado 2026-07-23). Prioridade: alta — sem isso o produto instalado não tem o padrão da casa.
...
- **Loop de aprendizado: knowledge base auto-enriquecida por deploy real** (ideia do usuário, 2026-07-23) — HOJE não existe: infra inédita que passa synth+deploy+teste NÃO volta pra base; o conhecimento se perde e o próximo pedido parecido começa do zero. Proposta: capturar deploys inéditos bem-sucedidos como CANDIDATOS a exemplo. Fluxo seguro (usa as peças de 2026-07-22/23): (1) generalizar o projeto (tirar nomes/lógica específicos, virar forma reutilizável); (2) deduplicar contra o corpus (padrão novo vs variação); (3) passar pelo HARNESS de contrato (synth + invariantes) — só entra se verde; (4) `validated:true` só se deploy-validado; (5) escrever no REPO (via PR/revisão), NUNCA direto no banco, pra preservar a fonte única. Sem os gates (harness, validated, fonte única) isso reintroduziria o rot dos 112 purgados. É a evolução natural pós-fonte-única. Prioridade: média-alta (multiplica o valor da ferramenta com o uso).
