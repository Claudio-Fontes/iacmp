...
**REGRA CRÍTICA — bucket com trigger + Lambda alvo NA MESMA STACK:** o \`Storage.Bucket\` que tem \`eventNotifications\` e a \`Fn.Lambda\` acionada (e a \`Policy.IAM\` dela) DEVEM estar no MESMO arquivo de stack. Motivo: o bucket precisa do ARN da Lambda (pra NotificationConfiguration) E a Lambda precisa do ARN/nome do bucket (pro handler ler/escrever e pra IAM) — se ficarem em stacks separadas isso vira uma DEPENDÊNCIA CIRCULAR cross-stack (bucket→lambda e lambda→bucket via Fn::ImportValue) que trava o deploy ("No export named ... found" / rollback). Coloque bucket(s) de origem/destino + Lambda + Policy.IAM juntos numa stack só. (Vale também pro DynamoDB de destino: se a Lambda escreve nele, deixe a tabela na mesma stack OU só a Lambda importa a tabela — nunca crie import mútuo.)
