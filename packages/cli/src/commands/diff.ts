...
function synthStack(stack: Stack, provider: string, allStacks: Stack[], projectName?: string): string {
  switch (provider) {
    ...
    case 'gcp': {
      // Só resource{}/output{} — espelha o que `synth` grava em `<stack>.tf.json`
      // agora (os blocos terraform/provider/variable saem em `_providers.tf.json`,
      // comparados à parte mais abaixo, fora deste loop por stack). allStacks é
      // necessário aqui (cross-stack refs resolvem contra o registro do projeto inteiro).
      const p = new GCPProvider();
      return p.synthesizeResources(stack, allStacks);
    }
    case 'terraform': {
...
export default class Diff extends Command {
  ...
  async run(): Promise<void> {
    ...
    // GCP: blocos compartilhados do projeto inteiro (terraform/provider/variable),
    // gravados uma vez em `_providers.tf.json` — não são de nenhuma stack
    // individual, então ficam fora do loop acima (ver synth.ts synthesizeProviders).
    if (provider === 'gcp' && !flags.stack && loadedStacks.length > 0) {
      const providersPath = path.join(outDir, '_providers.tf.json');
      if (fs.existsSync(providersPath)) {
        const oldText = fs.readFileSync(providersPath, 'utf-8');
        const newText = new GCPProvider().synthesizeProviders(allStacks);
        this.log(`\n${chalk.bold('_providers')}${ext}`);
        this.log(chalk.dim('─'.repeat(50)));
        const hasDiff = renderDiff(oldText, newText);
        if (hasDiff) {
          anyDiff = true;
