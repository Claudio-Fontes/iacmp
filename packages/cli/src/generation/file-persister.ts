...
import { AIGeneratedResponse, requireAi } from '../pro';
...
export async function persistInitial(
  ...
  const previouslyWritten = await requireAi().writeGeneratedFiles(parsed.files, cwd, dryRun, ask);
  if (!dryRun && preExistingGeneratedFiles.length > 0) {
    const staleOrphans = requireAi().removeOrphanedGeneratedFiles(preExistingGeneratedFiles, parsed.files, cwd);
...
export async function rewriteAndReconcile(
  ...
  const written = await requireAi().writeGeneratedFiles(parsed.files, cwd, false, async () => 'y');
  if (written.length === 0) return previouslyWritten;
  const orphans = requireAi().removeOrphanedGeneratedFiles(previouslyWritten, parsed.files, cwd);
  if (orphans.length > 0) {
