...
import { AIProvider, ChatSession, AIGeneratedResponse, requireAi } from '../pro';
...
async function obtainRawResponse(
  ...
  const cached = requireAi().getCached(cwd, lastUserPrompt);
  if (cached) {
    try {
      requireAi().extractResponse(cached);
...
async function applySemanticReview(
  ...
  try {
    ...
    try {
      const reviewed = requireAi().extractResponse(reviewRaw);
...
async function fixInitialTypeErrors(
  ...
  try {
    ...
    try {
      const retryParsed = requireAi().extractResponse(retryRaw);
...
async function runSynthCorrectionLoop(
  ...
  for (let attempt = 1; attempt <= MAX_SYNTH_RETRIES; attempt++) {
    ...
    const { success, output } = requireAi().runSynthCapture(cwd, iacProvider);
    ...
    try {
      ...
      session.addAssistantMessage(retryRaw);
      try {
        parsed = requireAi().extractResponse(retryRaw);
...
export async function runGeneration(
  ...
  try {
    parsed = requireAi().extractResponse(raw);
  } catch {
  ...
  if (!fromCache) {
    requireAi().setCache(cwd, lastUserPrompt, raw);
  }
  ...
  requireAi().printExplanation(parsed.explanation);
  requireAi().printWarnings(parsed.warnings);
  ...
  requireAi().printNextSteps(parsed.nextSteps);
