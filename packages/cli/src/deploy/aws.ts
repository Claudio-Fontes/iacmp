...
export const awsExecutor: DeployExecutor = {
  ...
  async planDeploy(ctx: DeployContext): Promise<NativeCommand[]> {
    ...
    // --dry-run é OFFLINE: não chama a AWS (sts/describe-stacks) nem executa
    // efeitos (o bloco de rollback abaixo faz delete-stack real). Usa placeholder
    // no account id do bucket e assume stack inexistente.
    const accountId = ctx.dryRun ? '<ACCOUNT_ID>' : getAccountId();
    const bucket = artifactBucketName(accountId, ctx.region);
    const commands: NativeCommand[] = [];

    const existingStatus = ctx.dryRun ? { deployed: false } : describeStackStatus(ctx.stackName, ctx.region);
    ...
    if (ctx.dryRun || !bucketExists(bucket)) {
