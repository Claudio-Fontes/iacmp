...
export function templateExt(provider: string): string {
  if (provider === 'terraform' || provider === 'gcp' || provider === 'azure-tf') return '.tf.json';
...
export function providerOutDir(cwd: string, provider: string): string {
  const dir = provider === 'terraform' ? 'aws-tf' : provider;
  return path.join(synthRoot(cwd), dir);
}
...
export function countResources(filePath: string, provider: string): number {
  ...
  if (provider === 'terraform' || provider === 'gcp' || provider === 'azure-tf') {
