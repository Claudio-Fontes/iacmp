...
describe('getAccountId', () => {
  ...
  test('retorna o Account retornado por `aws sts get-caller-identity`', () => {
    mockedCp.execFileSync.mockReturnValue('123456789012\n' as any);
    expect(getAccountId()).toBe('123456789012');
  });
...
describe('awsExecutor.planDeploy', () => {
  ...
  function mockAccountAndBucket(opts: { accountId?: string; bucketExists?: boolean } = {}) {
    const { accountId = '123456789012', bucketExists: exists = true } = opts;
    mockedCp.execFileSync.mockImplementation((bin: string, args?: readonly string[]) => {
  ...
  test('monta s3 mb (bucket ausente) + package + deploy, na ordem certa', async () => {
    ...
    expect(commands[0].args).toEqual(['s3', 'mb', 's3://iacmp-deploy-artifacts-123456789012-us-east-1', '--region', 'us-east-1']);
    expect(commands[1].args).toEqual(
      expect.arrayContaining(['cloudformation', 'package', '--template-file', ctx.templatePath, '--s3-bucket', 'iacmp-deploy-artifacts-123456789012-us-east-1', '--region', 'us-east-1'])
    );
