...
describe('deploy --dry-run — caso feliz por provider (synth antes)', () => {
  ...
  test('aws --format tf: opera no diretório inteiro (terraform init + apply), não por stack', () => {
    dir = makeProject({ provider: 'aws' });
    const synth = runCli(['synth', '--provider', 'aws', '--format', 'tf'], { cwd: dir });
    expect(synth.status).toBe(0);
    expect(exists(dir, 'synth-out/aws-tf/main-stack.tf.json')).toBe(true);

    const r = runCli(['deploy', '--provider', 'aws', '--format', 'tf', '--dry-run'], { cwd: dir });
    expect(r.status).toBe(0);
  ...
  test('aws --format tf: --stack não é suportado', () => {
    ...
    const r = runCli(['deploy', '--provider', 'aws', '--format', 'tf', '--stack', 'main-stack', '--dry-run'], { cwd: dir });
    expect(r.status).not.toBe(0);
