...
describe('synth — comando (black-box)', () => {
  ...
  describe('providers nativos: caminho, extensão e conteúdo válido', () => {
    ...
    test('gcp → synth-out/gcp/main-stack.tf.json com JSON Terraform válido', () => {
      ...
      // O .tf.json por stack tem só resource{}/output{} — terraform/provider/
      // variable saem UMA vez em _providers.tf.json (compartilhado do projeto
      // inteiro), senão um projeto com 2+ stacks duplicaria esses blocos e
      // `terraform validate` no diretório inteiro abortaria com "Duplicate
      // required providers/provider/variable configuration".
      const parsed = JSON.parse(read(dir, 'synth-out/gcp/main-stack.tf.json'));
      expect(parsed).not.toHaveProperty('terraform');
      expect(parsed).toHaveProperty('resource');

      expect(r.stdout).toContain(path.join('synth-out', 'gcp', '_providers.tf.json'));
      expect(exists(dir, 'synth-out/gcp/_providers.tf.json')).toBe(true);
      const providers = JSON.parse(read(dir, 'synth-out/gcp/_providers.tf.json'));
      expect(providers).toHaveProperty('terraform');
      expect(providers).toHaveProperty('provider');
      expect(providers).toHaveProperty('variable');
      expect(providers).not.toHaveProperty('resource');
    });

    test('--provider terraform standalone foi desabilitado: erro orientando --format tf', () => {
    ...
    test('aws --format tf → synth-out/aws-tf/main-stack.tf.json com JSON válido', () => {
      dir = makeProject({ provider: 'aws' });
      const r = runCli(['synth', '--provider', 'aws', '--format', 'tf'], { cwd: dir });

      expect(r.status).toBe(0);
      expect(r.stdout).toContain(path.join('synth-out', 'aws-tf', 'main-stack.tf.json'));
      // extensão correta: .tf.json (Terraform JSON syntax)
      expect(exists(dir, 'synth-out/aws-tf/main-stack.tf.json')).toBe(true);
      expect(exists(dir, 'synth-out/aws-tf/main-stack.json')).toBe(false);
      // blocos compartilhados (terraform/provider/variable) vão UMA vez no _providers.tf.json
      expect(exists(dir, 'synth-out/aws-tf/_providers.tf.json')).toBe(true);

      const tfJson = read(dir, 'synth-out/aws-tf/main-stack.tf.json');
      // TF JSON syntax: parseável e contém chave "resource"
      const parsed = JSON.parse(tfJson);
      expect(parsed).toHaveProperty('resource');
      // stack não repete os blocos compartilhados do _providers.tf.json
      expect(parsed).not.toHaveProperty('terraform');
  ...
  describe('default e flag de provider', () => {
    ...
    test('-p (alias curto) funciona igual a --provider', () => {
      dir = makeProject({ provider: 'aws' });
      const r = runCli(['synth', '-p', 'gcp'], { cwd: dir });
  ...
  describe('caminhos de erro', () => {
    ...
    test('provider inexistente: erro listando os disponíveis', () => {
      ...
      expect(r.all.toLowerCase()).toContain('não encontrado');
      // sugere os providers nativos ('terraform' standalone foi desabilitado)
      expect(r.all).toContain('aws');
      expect(r.all).toContain('gcp');
