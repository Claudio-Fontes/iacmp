...
describe('validateHandlerCloudSdk — dois mundos em synth-time (caso main1)', () => {
  ...
  test('mistura: dynamo (shim ok) + sns (sem shim) → erro só cita o sns', () => {
    write('worker.ts', [
      "import { DynamoDBClient } from '@aws-sdk/client-dynamodb';",
    ...
    const errs = validateHandlerCloudSdk(dir, 'azure');
    expect(errs.length).toBe(1);
    expect(errs[0]).toContain('@aws-sdk/client-sns');
    expect(errs[0]).not.toContain('client-dynamodb (');
  });

  test('sqs é dual-cloud (handler guarda isAzure) → SEM erro mesmo sem shim', () => {
    write('worker.ts', [
      "import { DynamoDBClient } from '@aws-sdk/client-dynamodb';",
