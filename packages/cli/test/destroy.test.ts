...
describe('destroy --dry-run', () => {
  ...
  test('aws --format tf: opera no diretório inteiro (terraform destroy), não por stack', () => {
    dir = makeProject({ provider: 'aws' });
    expect(runCli(['synth', '--provider', 'aws', '--format', 'tf'], { cwd: dir }).status).toBe(0);
    const r = runCli(['destroy', '--provider', 'aws', '--format', 'tf', '--dry-run'], { cwd: dir });
    expect(r.status).toBe(0);
  ...
  test('aws --format tf: --stack não é suportado', () => {
    dir = makeProject({ provider: 'aws' });
    expect(runCli(['synth', '--provider', 'aws', '--format', 'tf'], { cwd: dir }).status).toBe(0);
    const r = runCli(['destroy', '--provider', 'aws', '--format', 'tf', '--stack', 'main-stack', '--dry-run'], { cwd: dir });
    expect(r.status).not.toBe(0);
    expect(r.all).toContain('--stack não é suportado com --format tf');
  });
...
describe('destroy — confirmação interativa (sem --force, sem --dry-run)', () => {
  ...
  test('input "y" prossegue e chega na checagem da CLI nativa (terraform não instalado em CI)', () => {
    ...
    // describeStatus do GCP considera "deployado" quando o tfstate compartilhado
    // tem recursos — sem isso, o destroy pula a stack ("não deployada") antes de
    // tocar o terraform. Simula um deploy prévio escrevendo um tfstate com 1 recurso.
    fs.writeFileSync(
      path.join(dir, 'synth-out', 'gcp', 'terraform.tfstate'),
      JSON.stringify({ resources: [{ type: 'google_storage_bucket', instances: [{ attributes: {} }] }] }),
    );
