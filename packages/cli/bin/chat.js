...
// @iacmp/ai é módulo Pro (repo privado). O chat só é spawnado por `iacmp ai
// --chat`, que já passou pelo gate — mas se rodado direto sem o Pro, falha
// com a mensagem certa em vez de um stack trace de require.
let aiModule;
try {
  aiModule = require('@iacmp/ai');
} catch {
  console.error('O modo chat faz parte do iacmp Pro — o módulo @iacmp/ai não está instalado.');
  process.exit(1);
}
const {
