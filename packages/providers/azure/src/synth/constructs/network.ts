...
export function synthesizeNetwork(construct: BaseConstruct, ctx: SynthContext): void {
  ...
  switch (construct.type) {
    ...
    case 'Network.WAF': {
      ...
      if (apiId) {
        // apiId presente → App Gateway WAF_v2 em frente ao APIM.
        // Front Door é bloqueado em contas Free Trial; App Gateway WAF_v2 funciona e
        // requer subnet dedicada (/24 garante espaço suficiente).
        const wafPolicySym = sym;
        const vnetSym = `${sym}Vnet`;
        const pipSym = `${sym}Pip`;
        const appGwSym = `${sym}AppGw`;
        const appGwName = `${construct.id.toLowerCase()}-appgw`;

        // Mapa managedRule/managedGroup → OWASP p/ App Gateway WAF
        const AG_RULE_MAP: Record<string, { ruleSetType: string; ruleSetVersion: string }> = {
          'sqli-stable': { ruleSetType: 'OWASP', ruleSetVersion: '3.2' },
          'xss-stable': { ruleSetType: 'OWASP', ruleSetVersion: '3.2' },
          'common-rule-set': { ruleSetType: 'OWASP', ruleSetVersion: '3.2' },
        ...
        const agCustomRules = rules
          .filter(r => !r.managedGroup && !r.rateLimit && !r.managedRule && !((r.type as string ?? '').toLowerCase().includes('rate')))
          .map((r, i) => ({
        ...
        const agSeenRuleSets = new Set<string>();
        const agManagedRuleSets: Array<{ ruleSetType: string; ruleSetVersion: string }> = [];
        for (const r of rules) {
          const group = (r.managedGroup as string | undefined) ?? (r.managedRule as string | undefined);
          if (group) {
            const mapped = AG_RULE_MAP[group] ?? { ruleSetType: 'OWASP', ruleSetVersion: '3.2' };
            const key = `${mapped.ruleSetType}@${mapped.ruleSetVersion}`;
            if (!agSeenRuleSets.has(key)) { agSeenRuleSets.add(key); agManagedRuleSets.push(mapped); }
          }
        }
        if (agManagedRuleSets.length === 0) agManagedRuleSets.push({ ruleSetType: 'OWASP', ruleSetVersion: '3.2' });

        // 1. WAF Policy (App Gateway WAF)
        resources.push({
          sym: wafPolicySym,
          ...
          properties: {
            ...
            customRules: agCustomRules,
            managedRules: { managedRuleSets: agManagedRuleSets },
          },
        });

        // 2. VNet dedicada (App Gateway WAF_v2 exige subnet /24 sem delegation)
        resources.push({
          sym: vnetSym,
          type: 'Microsoft.Network/virtualNetworks',
          apiVersion: '2023-04-01',
          name: `${construct.id}-vnet`,
          location: 'location',
          tags: tag(construct.id),
          properties: {
            addressSpace: { addressPrefixes: ['10.200.0.0/16'] },
            subnets: [{ name: 'appgw-subnet', properties: { addressPrefix: '10.200.0.0/24' } }],
          },
        });

        // 3. Public IP Standard (obrigatório para WAF_v2)
        resources.push({
          sym: pipSym,
          type: 'Microsoft.Network/publicIPAddresses',
          apiVersion: '2023-04-01',
          name: `${construct.id}-pip`,
          location: 'location',
          tags: tag(construct.id),
          sku: { name: 'Standard' },
        ...
        // 4. APIM backend host (mesma stack ou cross-stack)
        let apimBackendFqdn: unknown;
        const apimConstruct = ctx.idx.get(apiId) ?? ctx.globalIdx?.get(apiId);
        if (apimConstruct) {
          const apimSym = toSym(apiId);
          apimBackendFqdn = expr(`replace(replace(${apimSym}.properties.gatewayUrl, 'https://', ''), '/', '')`);
        } else {
          const paramName = crossParamName(apiId, 'GatewayUrl');
          ctx.crossParams?.set(paramName, 'string');
          apimBackendFqdn = expr(`replace(replace(${paramName}, 'https://', ''), '/', '')`);
        }

        // 5. App Gateway WAF_v2 — frontend → Public IP → listener → backend (APIM)
        resources.push({
          sym: appGwSym,
          ...
          location: 'location',
          tags: tag(construct.id),
          dependsOn: [wafPolicySym, vnetSym, pipSym],
          properties: {
            sku: { name: 'WAF_v2', tier: 'WAF_v2', capacity: 1 },
            firewallPolicy: { id: expr(`${wafPolicySym}.id`) },
            gatewayIPConfigurations: [{
              name: 'appGwIpConfig',
              properties: { subnet: { id: expr(`'\${${vnetSym}.id}/subnets/appgw-subnet'`) } },
            }],
            frontendIPConfigurations: [{
              name: 'appGwFrontendIP',
              properties: { publicIPAddress: { id: expr(`${pipSym}.id`) } },
            }],
            frontendPorts: [{ name: 'port80', properties: { port: 80 } }],
            backendAddressPools: [{ name: 'apimPool', properties: { backendAddresses: [{ fqdn: apimBackendFqdn }] } }],
            backendHttpSettingsCollection: [{
              name: 'apimHttpSettings',
              properties: { port: 443, protocol: 'Https', requestTimeout: 30, pickHostNameFromBackendAddress: true, cookieBasedAffinity: 'Disabled' },
            }],
            httpListeners: [{
              name: 'apimListener',
              properties: {
                frontendIPConfiguration: { id: expr(`resourceId('Microsoft.Network/applicationGateways/frontendIPConfigurations', '${appGwName}', 'appGwFrontendIP')`) },
                frontendPort: { id: expr(`resourceId('Microsoft.Network/applicationGateways/frontendPorts', '${appGwName}', 'port80')`) },
                protocol: 'Http',
              },
            }],
            requestRoutingRules: [{
              ...
              properties: {
                priority: 100,
                ruleType: 'Basic',
                httpListener: { id: expr(`resourceId('Microsoft.Network/applicationGateways/httpListeners', '${appGwName}', 'apimListener')`) },
                backendAddressPool: { id: expr(`resourceId('Microsoft.Network/applicationGateways/backendAddressPools', '${appGwName}', 'apimPool')`) },
                backendHttpSettings: { id: expr(`resourceId('Microsoft.Network/applicationGateways/backendHttpSettingsCollection', '${appGwName}', 'apimHttpSettings')`) },
              },
            }],
          },
        });

        outputs.push({ name: outputName(construct.id, 'Url'), type: 'string', value: `${pipSym}.properties.ipAddress` });

      } else {
        // Sem apiId: WAF policy standalone (App Gateway WAF policy — não enforça sem App Gateway)
