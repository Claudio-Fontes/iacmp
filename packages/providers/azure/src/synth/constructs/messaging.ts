...
export function synthesizeMessaging(construct: BaseConstruct, ctx: SynthContext): void {
  ...
  switch (construct.type) {
    ...
    case 'Messaging.Stream': {
      const ehPfx = construct.id.toLowerCase().replace(/[^a-z0-9-]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '').slice(0, 30) || 'eh';
      const nsName = expr(`'${ehPfx}-\${uniqueString(resourceGroup().id)}'`);
      const nsSym = `${sym}Ns`;
      resources.push({
      ...
      resources.push({
        ...
        properties: {
          ...
          partitionCount: (props.shards as number) ?? (props.shardCount as number) ?? 2
        }
      });
