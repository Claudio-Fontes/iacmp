...
describe('AzureProvider (Bicep)', () => {
  ...
  test('Function.ApiGateway type WEBSOCKET → Microsoft.SignalRService/WebPubSub (não APIM)', () => {
    const stack = new Stack('test');
    new Fn.Lambda(stack, 'ConnectFn', { runtime: 'nodejs20', handler: 'connect.handler', code: '.' });
    new Fn.Lambda(stack, 'DisconnectFn', { runtime: 'nodejs20', handler: 'disconnect.handler', code: '.' });
    new Fn.Lambda(stack, 'DefaultFn', { runtime: 'nodejs20', handler: 'default.handler', code: '.' });
    ...
    const out = synth(stack);
    expect(out).not.toContain("'Microsoft.ApiManagement/service@");
    expect(out).toContain("'Microsoft.SignalRService/WebPubSub@");
    expect(out).toContain("'Microsoft.SignalRService/WebPubSub/hubs@");
    expect(out).toContain("systemEvents: [\n          'connect'\n        ]");
    expect(out).toContain("systemEvents: [\n          'connected'\n          'disconnected'\n        ]");
    expect(out).toContain("userEventPattern: '*'");
    expect(out).toContain('connectFn.properties.defaultHostName');
    expect(out).toContain('disconnectFn.properties.defaultHostName');
    expect(out).toContain('defaultFn.properties.defaultHostName');
    expect(out).toContain('output WsApiUrl string = wsApi.properties.hostName');
  });

  test('Function.ApiGateway type WEBSOCKET com lambdaId cross-stack → param FqdnParam em vez de símbolo local', () => {
    const stack = new Stack('ws-api');
    new Fn.ApiGateway(stack, 'WsApi', {
      name: 'websocket-api',
      type: 'WEBSOCKET',
      routes: [{ method: 'ANY', path: '$connect', lambdaId: 'ConnectFn' }],
    });
    const out = synth(stack);
    expect(out).toContain('param ConnectFnFqdn string');
    expect(out).toContain('${ConnectFnFqdn}');
  });
