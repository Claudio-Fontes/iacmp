...
import { extractGCPFunctionMeta } from '../src/synth/gcp-terraform';
...
describe('GCPProvider (Terraform google_*)', () => {
  ...
  test('Database.SQL sem vpcId → mantém IP público, sem private services access', () => {
    const stack = new Stack('test');
    new Database.SQL(stack, 'PublicDb', { engine: 'postgres' });
    const r = resources(synth(stack));
    const db: any = Object.values(r['google_sql_database_instance'])[0];
    expect(db.settings[0].ip_configuration).toBeUndefined();
    expect(db.depends_on).toBeUndefined();
    expect(r['google_compute_global_address']).toBeUndefined();
    expect(r['google_service_networking_connection']).toBeUndefined();
  });

  test('Database.SQL com vpcId → ip_configuration privado + private services access + depends_on', () => {
    const stack = new Stack('test');
    new Network.VPC(stack, 'Vpc1', {});
    new Database.SQL(stack, 'PrivDb', { engine: 'postgres', vpcId: 'Vpc1' } as any);
    const r = resources(synth(stack));
    const db: any = Object.values(r['google_sql_database_instance'])[0];
    expect(db.settings[0].ip_configuration[0]).toEqual({
      ipv4_enabled: false,
      private_network: '${google_compute_network.vpc1.id}',
    });
    expect(db.depends_on).toEqual(['google_service_networking_connection.vpc1_psa']);
    const globalAddr: any = r['google_compute_global_address']['vpc1_psa_range'];
    const connection: any = r['google_service_networking_connection']['vpc1_psa'];
    expect(globalAddr).toBeDefined();
    expect(globalAddr.purpose).toBe('VPC_PEERING');
    expect(connection.service).toBe('servicenetworking.googleapis.com');
  });

  test('Database.SQL: 2 instâncias na MESMA vpcId → dedup, um só par global_address+connection', () => {
    const stack = new Stack('test');
    new Network.VPC(stack, 'Vpc1', {});
    new Database.SQL(stack, 'PrivDbA', { engine: 'postgres', vpcId: 'Vpc1' } as any);
    new Database.SQL(stack, 'PrivDbB', { engine: 'mysql', vpcId: 'Vpc1' } as any);
    const r = resources(synth(stack));
    expect(Object.keys(r['google_compute_global_address'])).toEqual(['vpc1_psa_range']);
    expect(Object.keys(r['google_service_networking_connection'])).toEqual(['vpc1_psa']);
    expect(Object.keys(r['google_sql_database_instance'])).toHaveLength(2);
  });
  ...
  test('extractGCPFunctionMeta: Fn.Lambda sem eventSources → trigger "http"', () => {
    const stack = new Stack('test');
    new Fn.Lambda(stack, 'HttpFn', { runtime: 'nodejs20', handler: 'index.handler', code: 'dist/' });
    const [meta] = extractGCPFunctionMeta(stack);
    expect(meta.trigger).toBe('http');
  });

  test('extractGCPFunctionMeta: Fn.Lambda com eventSources[].queueId (Pub/Sub) → trigger "event"', () => {
    const stack = new Stack('test');
    ...
    new Fn.Lambda(stack, 'WorkerFn', {
      ...
      code: 'dist/',
      eventSources: [{ queueId: 'JobsQueue' }],
    });
    const meta = extractGCPFunctionMeta(stack).find((m) => m.constructId === 'WorkerFn');
    expect(meta?.trigger).toBe('event');
  });
