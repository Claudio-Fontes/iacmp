  ...
  switch (construct.type) {
    ...
    case 'Storage.Bucket': {
      ...
      addResource(r, 'google_storage_bucket', id, {
        // Nome de bucket GCS é GLOBAL (namespace compartilhado entre todos os
        // projetos/usuários) — prefixar com project_id garante unicidade, senão
        // dá 409 "bucket name is not available". Mesma convenção do bucket de
        // artefatos (deploy/gcp.ts: `${projectId}-artifacts`). O output e o env
        // BUCKET usam `${...name}`, então resolvem o nome real sozinhos.
        name: `\${var.project_id}-${construct.id.toLowerCase().replace(/[^a-z0-9-]/g, '-')}`,
        location: (props.location as string) ?? 'US',
        // `iacmp destroy` deve remover o bucket mesmo com objetos dentro (senão
        // o terraform erra: "containing objects without force_destroy"). Paridade
        // com o destroy AWS, que esvazia o S3 antes de deletar.
        force_destroy: true,
    ...
    case 'Storage.Archive': {
      ...
      addResource(r, 'google_storage_bucket', id, {
        name: `\${var.project_id}-${construct.id.toLowerCase().replace(/[^a-z0-9-]/g, '-')}-archive`,
        location: 'US',
        force_destroy: true,
