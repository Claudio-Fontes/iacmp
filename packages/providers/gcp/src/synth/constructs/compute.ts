...
export function synthCompute(construct: BaseConstruct, ctx: TFOutput): boolean {
  ...
  switch (construct.type) {
    ...
    case 'Compute.Container': {
      ...
      // props.build → placeholder substituído pelo deploy após gcloud builds submit.
      // props.image → URI de registro remoto (ex: gcr.io/proj/app:tag, docker.io/...).
      const buildCtx = props.build as { context?: string } | undefined;
      const imageUri = (props.image as string | undefined)
        ?? (buildCtx ? `gcr.io/IACMP_BUILD_PLACEHOLDER/${gcpName(construct.id)}:latest` : undefined);
      if (!imageUri) {
        console.warn(`[gcp] Compute.Container '${construct.id}' sem image nem build — ignorado.`);
        return true;
      }
      addResource(r, 'google_cloud_run_v2_service', id, {
        // Cloud Run service_id é RFC1035 (só minúsculas/dígitos/hífen, começa
        // com letra, <50 chars) — construct.id cru (PascalCase) é rejeitado.
        name: gcpName(construct.id),
      ...
      // publicIp: ingress ALL abre a REDE, mas o Cloud Run ainda exige IAM —
      // sem allUsers como run.invoker, qualquer chamada (inclusive via LB) leva
      // 403. Torna o serviço publicamente invocável quando publicIp:true.
      if (props.publicIp as boolean) {
        addResource(r, 'google_cloud_run_v2_service_iam_member', `${id}_public`, {
          name: `\${google_cloud_run_v2_service.${id}.name}`,
          location: '${var.gcp_region}',
          role: 'roles/run.invoker',
          member: 'allUsers',
        });
      }
      ctx.outputs[`${construct.id}ServiceUrl`] = {
        value: `\${google_cloud_run_v2_service.${id}.uri}`,
      };
      return true;
    }
