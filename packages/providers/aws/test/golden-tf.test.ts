/**
 * Golden Terraform tests — compara o output do emitTerraform com JSONs commitados.
 *
 * Para regenerar os goldens (após mudança intencional no emitter):
 *   UPDATE_GOLDEN=1 npm test --workspace=packages/providers/aws -- --testPathPattern=golden-tf
 *
...
import { emitTerraform } from '../src/synth/emit/terraform';
...
describe('Golden Terraform templates', () => {
  ...
  test('ecs-alb', () => {
    ...
    assertGolden('ecs-alb', emitTerraform(tpl));
  });
  ...
  test('lambda-vpc-redis', () => {
    ...
    assertGolden('lambda-vpc-redis', {
      'lvr-shared': emitTerraform(provider.synthesize(networkStack, allStacks)),
      'lvr-services': emitTerraform(provider.synthesize(servicesStack, allStacks)),
      'lvr-api': emitTerraform(provider.synthesize(computeStack, allStacks)),
    });
  });
  ...
  test('s3-lambda-pipeline', () => {
    ...
    const tpl = provider.synthesize(stack);
    assertGolden('s3-lambda-pipeline', emitTerraform(tpl));
  });
  ...
  test('stepfunctions-approval', () => {
    ...
    const tpl = provider.synthesize(stack);
    assertGolden('stepfunctions-approval', emitTerraform(tpl));
  });
  ...
  test('waf-rest-api', () => {
    ...
    const tpl = provider.synthesize(stack);
    assertGolden('waf-rest-api', emitTerraform(tpl));
  });
  ...
  test('websocket-api', () => {
    ...
    assertGolden('websocket-api', {
      'ws-data': emitTerraform(provider.synthesize(dataStack, allStacks)),
      'ws-api': emitTerraform(provider.synthesize(apiStack, allStacks)),
    });
  });
  ...
  test('kinesis-stream', () => {
    ...
    const tpl = provider.synthesize(stack);
    assertGolden('kinesis-stream', emitTerraform(tpl));
  });
  ...
  test('rds-secret', () => {
    ...
    assertGolden('rds-secret', {
      'rs-infra': emitTerraform(provider.synthesize(networkStack, allStacks)),
      'rs-db': emitTerraform(provider.synthesize(dbStack, allStacks)),
      'rs-compute': emitTerraform(provider.synthesize(computeStack, allStacks)),
    });
  });
  ...
  test('sns-alarm', () => {
    ...
    const tpl = provider.synthesize(stack);
    assertGolden('sns-alarm', emitTerraform(tpl));
  });
});
