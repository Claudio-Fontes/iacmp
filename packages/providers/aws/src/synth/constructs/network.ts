...
export function synthNetwork(
  ...
  switch (construct.type) {
    ...
    case 'Network.WAF': {
      ...
      // managedRule (singular) é alias de managedGroup — mapeia keywords agnósticas p/ AWS managed rule groups
      const MANAGED_RULE_MAP: Record<string, string> = {
        'sqli-stable': 'AWSManagedRulesSQLiRuleSet',
        'xss-stable': 'AWSManagedRulesCommonRuleSet',
        'common-rule-set': 'AWSManagedRulesCommonRuleSet',
      };
      const entries: Array<[string, CloudFormationResource]> = [[logicalId, {
        ...
        Properties: {
          ...
          Rules: rules.map((r, i) => {
            ...
            // managedRule alias → trata como managedGroup
            const resolvedManagedGroup = (r.managedGroup as string | undefined)
              ?? (r.managedRule ? MANAGED_RULE_MAP[r.managedRule as string] ?? r.managedRule as string : undefined);
            ...
            const statement = resolvedManagedGroup
              ? { ManagedRuleGroupStatement: { VendorName: 'AWS', Name: resolvedManagedGroup } }
              : r.rateLimit
      ...
      // apiId presente → associar o WAF ao stage da API via WebACLAssociation.
      // WAFv2 SÓ suporta REST API (v1); HTTP API (v2) é rejeitado com "ARN isn't valid".
      // ARN REST:  arn:aws:apigateway:{region}::/restapis/{id}/stages/prod
      // ARN HTTP:  arn:aws:apigateway:{region}::/apis/{id}/stages/$default  (não suportado)
      if (apiId) {
        const apiLogicalId = apiId.replace(/[^a-zA-Z0-9]/g, '');
        const isRest = (ctx.apiGwTypes.get(apiId) ?? 'HTTP') === 'REST';
        const stageName = isRest ? 'prod' : '$default';
        const stageArn = isRest
          ? `arn:aws:apigateway:\${AWS::Region}::/restapis/\${${apiLogicalId}}/stages/${stageName}`
          : `arn:aws:apigateway:\${AWS::Region}::/apis/\${${apiLogicalId}}/stages/${stageName}`;
        const assocId = `${logicalId}Association`;
        entries.push([assocId, {
          Type: 'AWS::WAFv2::WebACLAssociation',
          DependsOn: [logicalId, `${apiLogicalId}Stage`],
          Properties: {
            WebACLArn: { 'Fn::GetAtt': [logicalId, 'Arn'] },
            ResourceArn: { 'Fn::Sub': stageArn },
          },
        }]);
      }
      return entries;
