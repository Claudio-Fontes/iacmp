...
export function synthMonitoring(
  ...
  switch (construct.type) {
    case 'Monitoring.Alarm': {
      ...
      // notificationTopicId: string ID de um Messaging.Topic/Queue — SNS aceita ARN do tópico
      // como AlarmAction. Merge com alarmActions para reusar resolveAlarmAction (aceita id bare).
      const notificationTopicId = props.notificationTopicId as string | undefined;
      const rawAlarmActions = Array.isArray(props.alarmActions)
        ? props.alarmActions
        : props.alarmActions != null ? [props.alarmActions] : [];
      const mergedAlarmActions = notificationTopicId
        ? [...rawAlarmActions, notificationTopicId]
        : rawAlarmActions;
      return [[logicalId, {
        ...
        Properties: {
          ...
          ...alarmActionsBlock('AlarmActions', mergedAlarmActions, ctx),
