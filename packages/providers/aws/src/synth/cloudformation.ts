...
export function buildGraph(stack: Stack, allStacks?: Stack[], profile: EnvironmentProfile = DEFAULT_PROFILE, projectName?: string): StackGraph {
  ...
  const apiGwTypes = new Map<string, string>();
  ...
  for (const s of universe) {
    for (const c of s.constructs) {
      ...
      if (c.type === 'Function.ApiGateway') {
        ...
        apiGwTypes.set(c.id, (p.type as string) ?? 'HTTP');
      }
  ...
  const ctx: SynthContext = { currentStackName: prefixStack(stack.name), projectName, registry, apiGwTypes, lambdaRoles, vpcLambdas, dbSecretSuffix, dbMasterUsername, sqsEventSourceLambdas, kinesisEventSourceLambdas, albDefaultTg, publicSubnetsByVpc, profile, s3TriggerBucketsForLambda, sfnInitiatorLambdas, dynamoRefLambdas, sqsSenderRefLambdas };
  ...
  for (const construct of stack.constructs) {
    ...
    if (construct.type === 'Function.ApiGateway') {
      ...
      // (comportamento documentado só para HTTP API: "$default stage that is served
      // from the base of your API's URL"). WEBSOCKET não tem essa omissão — mesmo
      // com stage $default, o path da URL/callback sempre inclui o stage
      // (ver docs: callbackUrl = `https://${domain}/${stage}`), igual REST.
      const urlStagePath = apigwType === 'HTTP' ? '' : `/${stageName}`;
