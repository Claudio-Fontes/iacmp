    ...
    for (const c of s.constructs) {
      ...
      if (Array.isArray(props.routes)) {
        for (const r of props.routes as Array<Record<string, unknown>>) {
          ...
          // Alias errado: 'functionId' é silenciosamente ignorado pelo synth → APIM sem backend
          if (typeof (r as Record<string, unknown>)?.functionId === 'string') {
            errors.push(`Fn.ApiGateway route usa "functionId" mas o campo correto é "lambdaId" (routes[].functionId="${(r as Record<string, unknown>).functionId}"). Corrija para lambdaId.`);
          }
        }
      }
