...
# iacmp-pro (repo privado irmão): @iacmp/ai + @iacmp/knowledge vivem lá desde a
# separação open core. O CLI referencia via file: — o Pro precisa estar buildado
# ANTES do build do monorepo (o tsup resolve o dist/ do pacote).
PRO_DIR="$REPO_ROOT/../iacmp-pro"
if [ -d "$PRO_DIR" ]; then
  step "0/5 Build do iacmp-pro (ai + knowledge)"
  (cd "$PRO_DIR" && npm install --no-audit --no-fund 2>&1 | tail -1 && npm run build 2>&1 | tail -1) || fail "build do iacmp-pro falhou"
else
  printf '\nAVISO: %s não existe — comandos Pro (iacmp ai) não funcionarão.\n' "$PRO_DIR"
fi

step "1/5 Dependências (npm install se o lockfile mudou)"
npm install --no-audit --no-fund 2>&1 | tail -1

# Symlinks dos módulos Pro: @iacmp/ai e @iacmp/knowledge NÃO são deps do
# package.json (o CLI público não pode depender de pacote privado) — o CLI os
# carrega dinamicamente via src/pro/. Na máquina de dev, linkamos direto do
# checkout do iacmp-pro DEPOIS do npm install (que removeria links extraneous).
if [ -d "$PRO_DIR" ]; then
  mkdir -p "$REPO_ROOT/node_modules/@iacmp"
  ln -sfn "$PRO_DIR/packages/ai" "$REPO_ROOT/node_modules/@iacmp/ai"
  ln -sfn "$PRO_DIR/packages/knowledge" "$REPO_ROOT/node_modules/@iacmp/knowledge"
fi
